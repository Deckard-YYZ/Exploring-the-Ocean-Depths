...
public class CameraUnderWater : MonoBehaviour
{
    ...
    //the color transformation is based on this variable
    public float waterSurfaceHeight;
    public Shader shader;
    public Texture2D gradientTexture;
    [Header("Depth Effect")]
    public Color depthColor = new Color(0, 0.42f, 0.87f);
    public float depthStart = -12, depthEnd = 98, scale = 1.0f;
    ...
    void Update()
    {
        ...
        float position = transform.position.y/ waterSurfaceHeight; // set to this % of color
        int x = (int)(gradientTexture.width * position);
        depthColor = gradientTexture.GetPixel(x, 0);
    }
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        if (m_Material)
        {
            ...
            m_Material.SetFloat("_DepthStart", depthStart);
            m_Material.SetFloat("_DepthEnd", depthEnd);
            m_Material.SetFloat("_ToSeeSome", scale);
